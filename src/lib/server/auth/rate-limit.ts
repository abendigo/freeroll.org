...
// keyHash: same hmacHex(...) as users.email_hash for 'email', or of the raw IP for 'ip' — see
// hash.ts. This table never stores the plaintext of either.
...
export async function recordAttempt(db: Kysely<AppDatabase>, keyType: RateLimitKeyType, keyHash: string): Promise<void> {
	await db.insertInto('login_attempts').values({ key_type: keyType, key_hash: keyHash }).execute();
}
